...
public class DragListView extends ListView implements OnScrollListener,
    ...
    private enum DListViewState {
        LV_LOADING, // 普通状态
        LV_NORMAL, // 下拉状态（为超过mHeadViewHeight）
        LV_PULL_REFRESH, // 松开可刷新状态（超过mHeadViewHeight）
        LV_RELEASE_REFRESH;// 加载状态
    }
        ...
        switch (mlistViewState) {
            ...
            case LV_NORMAL: {
                ...
                if (offset > 0) {
                    ...
                    switchHeaderViewState(DListViewState.LV_PULL_REFRESH);// 下拉状态
                }
            ...
            case LV_PULL_REFRESH: {
                ...
                if (offset < 0) {
                     ...
                     */
                    isScroller = false;
                    switchHeaderViewState(DListViewState.LV_NORMAL);// 普通状态
                    Log.e("jj", "isScroller=" + isScroller);
                } else if (offset > mHeadViewHeight) {// 如果下拉的offset超过headView的高度则要执行刷新.
                    switchHeaderViewState(DListViewState.LV_RELEASE_REFRESH);// 更新为可刷新的下拉状态.
                }
            }
            break;
            // 可刷新状态
            case LV_RELEASE_REFRESH: {
                ...
                if (offset >= 0 && offset <= mHeadViewHeight) {
                    mBack = true;
                    switchHeaderViewState(DListViewState.LV_PULL_REFRESH);
                } else if (offset < 0) {
                    switchHeaderViewState(DListViewState.LV_NORMAL);
                } else {
        ...
        switch (mlistViewState) {
            ...
            case LV_PULL_REFRESH:
                if (headerViewEnable) {
                    ...
                    switchHeaderViewState(mlistViewState.LV_NORMAL);
                }
                break;
            // 刷新状态
            case LV_RELEASE_REFRESH:
                if (headerViewEnable) {
                    ...
                    switchHeaderViewState(mlistViewState.LV_LOADING);
                }
                onRefresh();// 下拉刷新
                break;
        }
    ...
    public void setHeadViewEnalbe(boolean enalbe) {
        if (this.headerViewEnable == enalbe) {
            return;
        }
        this.headerViewEnable = enalbe;
        if (headerViewEnable) {
            String time = getDataStr();
            initHeadView(context, time);// 初始化该head.
        } else {
    ...
    // 切换headview视图
    private void switchHeaderViewState(DListViewState state) {
        // if (true) {
        // return;
        // }
        switch (state) {
            ...
            case LV_NORMAL: {
                ...
                mArrowImageView.setImageResource(R.drawable.pull_to_refresh);
            }
            break;
            // 下拉状态
            case LV_PULL_REFRESH: {
                ...
                mLastUpdateTextView.setText("最近更新:" + dataStr);
                mArrowImageView.clearAnimation();// 清除动画
